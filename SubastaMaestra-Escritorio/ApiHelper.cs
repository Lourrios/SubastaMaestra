...
namespace SubastaMaestra_Escritorio
{
    public class ApiHelper
    {
        ...
        public static async Task<(T result, string errorMessage)> GetAsync<T>(string url)
        {
            try
            {
                var response = await _httpClient.GetAsync(url);
                ...
                {
                    var content = await response.Content.ReadAsStringAsync();
                    T result = typeof(T) == typeof(string) ? (T)(object)content : JsonConvert.DeserializeObject<T>(content);
